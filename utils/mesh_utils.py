...
from utils.render_utils import save_img_f32, save_img_u8, save_img_exr
...
class GaussianExtractor(object):
    ...
    @torch.no_grad()
    def get_gs_num(self):
        return int(self.gaussians._opacity.shape[0])
    ...
    @torch.no_grad()
    def extract_mesh_bounded(self, voxel_size=0.004, sdf_trunc=0.02, depth_trunc=3, mask_background=False):
        """
    ...
    def export_image(self, path):
        ...
        for idx, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="export images"):
            ...
            save_img_exr(self.normals[idx].permute(1,2,0).cpu().numpy() * 0.5 + 0.5, os.path.join(vis_path, 'normal_{0:05d}'.format(idx) + ".exr"))
