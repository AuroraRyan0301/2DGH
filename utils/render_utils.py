...
def save_img_exr(img, pth):
    height, width = img.shape[:2]

    file = OpenEXR.OutputFile(pth, OpenEXR.Header(width, height))

    red = img[:, :, 0]
    green = img[:, :, 1]
    blue = img[:, :, 2]

    float_to_half = Imath.PixelType(Imath.PixelType.HALF)

    file.writePixels({'R': red.tostring(),
                      'G': green.tostring(),
                      'B': blue.tostring()})
